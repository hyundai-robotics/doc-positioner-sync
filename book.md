# ${cont_model} 로봇제어기 기능설명서 - 포지셔너 동기
...
외부신호를 이용하여 별도로 등록된 move 명령어를 자동모드에서 구동하는 기능입니다. 이 기능을 사용하면 로봇의 동작과 독립적인 부가축 동작을 move 명령어를 이용하여 구현이 가능합니다. 이 기능을 사용하기 위해서는 메커니즘 설정과 명령문 독립실행 설정이 필요합니다.# 1.2 조작 순서
...
[**${cont_model} 로봇제어기 조작설명서 7.7장 자동 캘리브레이션**](https://hrbook-hrc.web.app/#/view/doc-hi6-operation/korean-tp630/7-setting/7-auto-calibration/README)을 참고하십시오.# 2.3 포지셔너 캘리브레이션
...
```
# 5. 부가축 move 독립 실행
부가축 move 독립 실행 기능은 외부입력 신호에 의해 부가축의 move 명령을 로봇과 독립적으로 실행할 수 있습니다.# 5.1 시스템 설정
